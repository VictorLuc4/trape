...
class Trape(object):
	def __init__(self, stat = 0):
		...
		if self.stat == 1:
			...
			self.ngrok = "1YewGNtlaqKzjXPdu0uwZlzA5my_3bDsYVD1PJxZmdFANxz8Y"
			self.gmaps = "AIzaSyAbiEcZCgk9rRlJ5loBHMLg4-R89VBLcCU"
			self.ipinfo = "ae6a755a61c34837a6767231296df1e6"
			if self.gmaps == '':
				self.gmaps = 'AIzaSyA30wEa2DwUuddmNTHvoprhnrB2w_aCWbs'
			self.googl = ''
			if self.googl == '':
				self.googl = 'AIzaSyDHMDTOGo9L1OBl5vRxOVM6vpXOXVp5Cc'
			...
			if (options.ngrok or (self.ngrok != "")):
				if self.ngrok == '':
					utils.Go("\033[H\033[J")
					self.ngrok = raw_input("What is your nGrok token? <-- should not appear (#123)" + " " + utils.Color['yellow'] + ":~> " + utils.Color['white'])
				if (self.ngrok != ''):
